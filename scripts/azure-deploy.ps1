<#
.SYNOPSIS
  One-time provisioning of devresume-api on Azure (Free Tier friendly).

.DESCRIPTION
  Creates:
    1. Resource Group
    2. Azure Database for PostgreSQL Flexible Server (Burstable B1ms / 32GB — the
       SKU covered by Azure's 12-months-free allowance)
    3. A GHCR-hosted image, built and pushed from the local Dockerfile
    4. The devresume-api Container App itself, wired up with DATABASE_URL / JWT_SECRET
  ...
  will error on resources that already exist (e.g. the Postgres server name), but
  won't silently duplicate anything.
...
# centralindia (check via `az policy assignment list -o json`). Postgres goes in
# centralindia — the same region as the existing nexuscart-env Container Apps
# environment, minimizing latency between the app and the database.
$PgLocation = "centralindia"

$PgServerName    = "devresume-pg-$(Get-Random -Minimum 1000 -Maximum 9999)"  # must be globally unique
...
Write-Host "`nGenerated Postgres admin password — save this now, it will not be shown again:" -ForegroundColor Yellow
Write-Host "  $PgAdminPassword`n" -ForegroundColor Yellow
...
# 1. Resource Group
# ---------------------------------------------------------------------------
az group create --name $ResourceGroup --location $PgLocation

# ---------------------------------------------------------------------------
# 2. PostgreSQL Flexible Server (Free-tier-eligible SKU)
#    --public-access 0.0.0.0 is a special sentinel meaning "allow public access
#    from any Azure service" — required because Container Apps on the
#    Consumption plan has no static outbound IP to allow-list individually.
# ---------------------------------------------------------------------------
...
# explicitly allow-listed first — the migrations use uuid-ossp and vector (pgvector),
# so without this, migrations fail on startup with "extension ... is not allow-listed".
...
# Azure Postgres Flexible Server enforces SSL by default — sslmode=require is mandatory.
$DatabaseUrl = "postgres://${PgAdminUser}:${PgAdminPassword}@${PgServerName}.postgres.database.azure.com:5432/${DbName}?sslmode=require"

# ---------------------------------------------------------------------------
# 3. Build & push the image to GHCR (free registry — avoids ACR's ~$5/mo Basic tier)
...
# ---------------------------------------------------------------------------
# 5. The Container App — deployed into the existing nexuscart-env (see NOTE above),
